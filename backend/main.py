...
MODEL_FILES = {
    ...
    "mental_roberta": ROOT / "modelo_mental_roberta_v2" / "best",
}
...
def assistant_reply(label: str, immediate_risk: bool, language: str) -> str:
    if language == "en":
        if immediate_risk:
...
def load_mental_roberta():
    ...
    path = str(ROOT / "modelo_mental_roberta_v2" / "best")
...
def analyze(request: AnalysisRequest):
    ...
    source_user_texts = [turn.text for turn in request.history if turn.role == "user"] + [request.text]
    ...
    probabilities = predict(current_text)
    ...
    immediate_risk = prediction == 2

    distribution = [
        {"label": label, "probability": round(float(probabilities[i]), 4)}
    ...
    return {
        ...
        "assistantReply": assistant_reply(LABELS[prediction], immediate_risk, request.language),
        "model": request.model,
...
def finalize(request: FinalizeRequest):
    user_indexes = [i for i, turn in enumerate(request.history) if turn.role == "user"]
    if not user_indexes:
        raise HTTPException(422, "La conversación no contiene mensajes del usuario.")
    last_user = request.history[user_indexes[-1]]
    result = analyze(AnalysisRequest(
        text=last_user.text,
        model=request.model,
        history=request.history[:user_indexes[-1]],
